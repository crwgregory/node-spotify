...
void PlaylistContainerCallbacksHolder::call(std::unique_ptr<Nan::Callback>& callback, std::initializer_list<Handle<Value>> args) {
...
void PlaylistContainerCallbacksHolder::playlistAdded(sp_playlistcontainer* pc, sp_playlist* spPlaylist, int position, void* userdata) {
  ...
  holder->call(holder->playlistAddedCallback, {NanUndefined(), playlistV8, Nan::New<Number>(position)});
}
...
void PlaylistContainerCallbacksHolder::playlistRemoved(sp_playlistcontainer* pc, sp_playlist* spPlaylist, int position, void *userdata) {
  ...
  if(nodePlaylist != nullptr) {
    holder->call(holder->playlistRemovedCallback, {NanUndefined(), Nan::New<Number>(position), NanObjectWrapHandle(nodePlaylist)});
  } else {
    holder->call(holder->playlistRemovedCallback, {NanUndefined(), Nan::New<Number>(position)});
  }
}
...
void PlaylistContainerCallbacksHolder::playlistMoved(sp_playlistcontainer* pc, sp_playlist* spPlaylist, int position, int new_position, void *userdata) {
  ...
  node::ObjectWrap* nodePlaylist = nullptr; //FIXME what?
  if(nodePlaylist != nullptr) {
    holder->call(holder->playlistMovedCallback, {NanUndefined(), Nan::New<Number>(position), Nan::New<Number>(new_position), NanObjectWrapHandle(nodePlaylist)});
  } else {
    holder->call(holder->playlistMovedCallback, {NanUndefined(), Nan::New<Number>(position), Nan::New<Number>(new_position)});
  }
}
