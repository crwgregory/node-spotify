...
void PlaylistCallbacksHolder::tracksAdded(sp_playlist* spPlaylist, sp_track *const *tracks, int num_tracks, int position, void *userdata) {
  ...
  holder->call(holder->tracksAddedCallback, { Nan::Undefined(), NanObjectWrapHandle(holder->userdata), nodeTracks, Nan::New<Number>(position) });
}
...
void PlaylistCallbacksHolder::tracksMoved(sp_playlist* spPlaylist, const int* tracks, int num_tracks, int new_position, void *userdata) {
  ...
  holder->call(holder->tracksMovedCallback, { Nan::Undefined(), NanObjectWrapHandle(holder->userdata), movedTrackIndices, Nan::New<Number>(new_position) });
}
...
void PlaylistCallbacksHolder::trackCreatedChanged(sp_playlist* spPlaylist, int position, sp_user* spUser, int when, void* userdata) {
  ...
  holder->call(holder->trackCreatedChangedCallback, { Nan::Undefined(), NanObjectWrapHandle(holder->userdata), Nan::New<Integer>(position), NanObjectWrapHandle(nodeUser), Nan::New<Date>(date) });
}

void PlaylistCallbacksHolder::trackSeenChanged(sp_playlist* spPlaylist, int position, bool seen, void* userdata) {
  auto holder = static_cast<PlaylistCallbacksHolder*>(userdata);
  holder->call(holder->trackSeenChangedCallback, { Nan::Undefined(), NanObjectWrapHandle(holder->userdata), Nan::New<Integer>(position), Nan::New<Boolean>(seen) });
}

void PlaylistCallbacksHolder::trackMessageChanged(sp_playlist* spPlaylist, int position, const char* message, void* userdata) {
  auto holder = static_cast<PlaylistCallbacksHolder*>(userdata);
  holder->call(holder->trackMessageChangedCallback, { Nan::Undefined(), NanObjectWrapHandle(holder->userdata), Nan::New<Integer>(position), Nan::New<String>(message) });
}
